...
const FLOWERS_LEFT: FlowerDef[] = [
  // main arm
  { x:  68, y: 188, r: 0.90, rot: -15 },
  { x: 108, y: 160, r: 0.84, rot:  22 },
  { x: 152, y: 136, r: 0.80, rot: -28 },
  { x: 198, y: 118, r: 0.76, rot:  38 },
  { x: 246, y: 108, r: 0.72, rot: -12 },
  { x: 292, y: 102, r: 0.70, rot:  50 },
  { x: 338, y:  98, r: 0.68, rot: -40 },
  // upper fork (goes up-left from mid arm)
  { x:  90, y: 112, r: 0.74, rot:  18 },
  { x:  62, y:  80, r: 0.70, rot: -32 },
  { x:  46, y:  48, r: 0.66, rot:  55 },
  // side sub-branch going down-right
  { x: 220, y: 148, r: 0.72, rot:  28 },
  { x: 258, y: 164, r: 0.68, rot: -20 },
  { x: 296, y: 178, r: 0.64, rot:  42 },
  // drooping twig cluster
  { x: 170, y: 162, r: 0.66, rot: -45 },
  { x: 136, y: 178, r: 0.62, rot:  14 },
  // far right extension
  { x: 374, y: 108, r: 0.64, rot: -18 },
  { x: 408, y: 122, r: 0.60, rot:  35 },
  // small outliers
...
export function SakuraCorner() {
  ...
  return (
    <>
      ...
      {/* ── LEFT BRANCH — enters from left, higher up, organic curves ── */}
      <div className="sakura-branch-wrap sakura-branch-left" aria-hidden="true">
        <svg ref={leftRef} viewBox="0 0 600 400" fill="none"
          ...
          {/* Primary arm — enters left edge at y=218, irregular S-curve right and slightly up */}
          <Branch specs={[
            ["M0 218 C32 224, 58 218, 88 206 C116 196, 140 182, 162 170 C182 160, 200 152, 222 144 C246 136, 272 130, 298 126 C322 122, 346 122, 368 124", 8.0, "5a3e28", 0.62],
            ["M0 218 C32 224, 58 218, 88 206 C116 196, 140 182, 162 170 C182 160, 200 152, 222 144 C246 136, 272 130, 298 126 C322 122, 346 122, 368 124", 4.8, "5a3e28", 0.74],
            ["M0 218 C32 224, 58 218, 88 206 C116 196, 140 182, 162 170 C182 160, 200 152, 222 144 C246 136, 272 130, 298 126 C322 122, 346 122, 368 124", 1.9, "3d2818", 0.86],
          ]} />

          {/* Upper fork — breaks upward from ~x=112, y=192, curves left then up sharply */}
          <Branch specs={[
            ["M112 192 C106 172, 98 148, 88 124 C80 104, 70 82, 62 60 C56 44, 50 28, 44 14", 5.2, "5a3e28", 0.58],
            ["M112 192 C106 172, 98 148, 88 124 C80 104, 70 82, 62 60 C56 44, 50 28, 44 14", 3.0, "5a3e28", 0.70],
            ["M112 192 C106 172, 98 148, 88 124 C80 104, 70 82, 62 60 C56 44, 50 28, 44 14", 1.2, "3d2818", 0.82],
          ]} />

          {/* Side sub-branch — arcs gently down-right from ~x=228, y=140 */}
          <Branch specs={[
            ["M228 140 C244 148, 260 158, 274 166 C288 174, 300 182, 310 192", 3.6, "5a3e28", 0.56],
            ["M228 140 C244 148, 260 158, 274 166 C288 174, 300 182, 310 192", 2.1, "5a3e28", 0.68],
            ["M228 140 C244 148, 260 158, 274 166 C288 174, 300 182, 310 192", 0.95, "3d2818", 0.80],
          ]} />

          {/* Far-right extension from ~x=368, y=124 — continues with a slight upturn */}
          <Branch specs={[
            ["M368 124 C388 120, 408 118, 428 120", 2.8, "5a3e28", 0.52],
            ["M368 124 C388 120, 408 118, 428 120", 1.6, "5a3e28", 0.64],
            ["M368 124 C388 120, 408 118, 428 120", 0.75, "3d2818", 0.76],
          ]} />

          {/* Short twig off upper fork at ~x=90, y=116 going right */}
          <Branch specs={[
            ["M90 116 C104 110, 118 108, 130 110", 1.8, "5a3e28", 0.50],
            ["M90 116 C104 110, 118 108, 130 110", 1.0, "3d2818", 0.64],
          ]} />

          {/* Twigs — each near a flower */}
          {[
            "M76 196 C70 194, 68 191, 68 188",       // f0
            "M118 168 C114 166, 111 163, 108 160",    // f1
            "M160 142 C156 140, 153 138, 152 136",    // f2
            "M204 122 C201 120, 199 119, 198 118",    // f3
            "M250 110 C248 108, 247 108, 246 108",    // f4
            "M296 104 C294 102, 293 102, 292 102",    // f5
            "M342 100 C340 98, 338 98, 338 98",       // f6
            "M92 118 C91 115, 91 113, 90 112",        // f7 (upper fork)
            "M64 82 C62 80, 61 79, 62 80",            // f8
            "M46 50 C44 48, 45 48, 46 48",            // f9
            "M224 146 C222 148, 221 148, 220 148",    // f10
            "M260 166 C258 165, 258 164, 258 164",    // f11
            "M298 180 C296 178, 296 178, 296 178",    // f12
            "M172 164 C171 162, 170 162, 170 162",    // f13
            "M138 180 C136 178, 136 178, 136 178",    // f14
            "M376 110 C374 108, 374 108, 374 108",    // f15
            "M410 124 C408 122, 408 122, 408 122",    // f16
            "M118 198 C116 196, 116 196, 116 196",    // f17
            "M322 118 C320 116, 320 116, 318 116",    // f18
          ].map((d, i) => (
            <path key={i} d={d} stroke="#4a3020" strokeWidth="0.85"
              strokeLinecap="round" fill="none" opacity="0.65" />
          ))}
